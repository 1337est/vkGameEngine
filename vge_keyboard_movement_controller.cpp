    ...
    VgeGameObject& gameObject)
{
    ...
    if (glm::dot(rotate, rotate) > std::numeric_limits<float>::epsilon())
    {
        gameObject.m_transform.rotation +=
            m_lookSpeed * dt * glm::normalize(rotate);
    }

    // limits pitch values between about +/- 85ish degrees
    gameObject.m_transform.rotation.x =
        glm::clamp(gameObject.m_transform.rotation.x, -1.5f, 1.5f);
    ...
    {
        gameObject.m_transform.translation +=
            m_moveSpeed * dt * glm::normalize(moveDir);
    }
}
} // namespace vge
